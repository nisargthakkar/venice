...
import com.linkedin.venice.init.SystemSchemaInitializer;
...
public class LeaderControllerSystemSchemaInitializer implements SystemSchemaInitializer {
  ...
  public LeaderControllerSystemSchemaInitializer(AvroProtocolDefinition protocolDefinition, VeniceControllerMultiClusterConfig multiClusterConfigs, VeniceHelixAdmin admin) {
    this(protocolDefinition, multiClusterConfigs, admin, Optional.empty(), Optional.empty(), false);
  }

  public LeaderControllerSystemSchemaInitializer(AvroProtocolDefinition protocolDefinition,
  ...
  public void execute() {
    ...
    try {
      ...
      if (!cluster.equals(systemSchemaClusterName)) {
        LOGGER.warn("The system store for '" + protocolDefinition.name() + "' already exists in cluster '"
            ...
            + "' which specifies that it should be in cluster '" + systemSchemaClusterName
      ...
      Store store = admin.getStore(systemSchemaClusterName, systemStoreName);
      if (null == store) {
        /**
        ...
        admin.createStore(systemSchemaClusterName, systemStoreName, VeniceConstants.SYSTEM_STORE_OWNER, firstKeySchemaStr,
            firstValueSchemaStr, true);
        // Update the default store config
        storeMetadataUpdate.ifPresent(
            updateStoreQueryParams -> admin.updateStore(systemSchemaClusterName, systemStoreName, updateStoreQueryParams));
    ...
    if (keySchema.isPresent()) {
       ...
       */
      SchemaEntry keySchemaEntry = admin.getKeySchema(systemSchemaClusterName, systemStoreName);
      if (!keySchemaEntry.getSchema().equals(keySchema.get())) {
        LOGGER.error("Key Schema of '" + systemStoreName + "' in cluster: " + systemSchemaClusterName +
     ...
     */
    Collection<SchemaEntry> schemaEntries = admin.getValueSchemas(systemSchemaClusterName, systemStoreName);
    ...
    for (int schemaVersion = 1; schemaVersion <= protocolDefinition.getCurrentProtocolVersion(); schemaVersion++) {
      ...
      if (null == knownSchema) {
        try {
          admin.addValueSchema(systemSchemaClusterName, systemStoreName, schemaInLocalResources.toString(), schemaVersion,
      ...
      if (autoRegisterDerivedComputeSchema) {
        ...
        Pair<Integer, Integer> derivedSchemaInfo = admin.getDerivedSchemaId(systemSchemaClusterName, systemStoreName, derivedSchema);
        if (derivedSchemaInfo.getFirst() == SchemaData.INVALID_VALUE_SCHEMA_ID) {
           ...
           */
          try {
            admin.addDerivedSchema(systemSchemaClusterName, systemStoreName, schemaVersion, derivedSchema);
          } catch (Exception e) {
  ...
  public String toString() {
    return "LeaderControllerSystemSchemaInitializer{" + "protocolDefinition=" + protocolDefinition + '}';
  }
}
